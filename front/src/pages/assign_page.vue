...
<template>
  <q-page>
    <q-table 
      ...
      <template v-slot:top="props">
        ...
        <div class="col-3 q-table__title">assign</div>
  ...
  <GridOptionsEditor ref="GridOptionsView"
      ...
      :columns="columns" optionName="assignpage">
  </GridOptionsEditor>
  <ObjectDisplay ref="object_display">
  </ObjectDisplay>
  <AssignEdit ref="edit_form" @hide="refresh_grid()" @ok="refresh_grid()">
  </AssignEdit>
  <AssignInsert ref="insert_form" @hide="refresh_grid()" @ok="refresh_grid()">
  </AssignInsert>
...
import AssignEdit from 'src/components/forms/AssignEdit.vue'
import AssignInsert from 'src/components/forms/AssignInsert.vue'
...
const columns = [
    ...
    },
    {
        ...
        label: 'assign_moment',
        align: 'left',
        field: row => row.assign_moment,
        format: val => `${val}`,
    },
    {
        name: 'assign_pagenumber',
        sortable: true,
        label: 'assign_pagenumber',
        align: 'left',
        field: row => row.assign_pagenumber,
        format: val => `${val}`,
    },
    {
        name: 'assign_status',
        ...
        label: 'assign_status',
        align: 'left',
        field: row => row.assign_status,
        format: val => `${val}`,
    },
    {
        name: 'assign_done_moment',
        sortable: true,
        label: 'assign_done_moment',
        align: 'left',
        field: row => row.assign_done_moment,
        format: val => `${val}`,
    },
    {
        name: 'assign_follow_result',
        sortable: true,
        label: 'assign_follow_result',
        align: 'left',
        field: row => row.assign_follow_result,
        format: val => `${val}`,
    },
]
...
export default {
  ...
  setup(props) {
    ...
    function onRequest(props) {
      ...
      const readUrl = '/assign/read'
      loading.value = true

      let query_params = _.defaults(Thinginfo.query_params.assign ?? {},
      {
        filter,
        page,
        pageSize: rowsPerPage,
        order_by: JSON.stringify([[sortBy ? sortBy : 'id', descending ? 'desc' : 'asc']]),
        load_1_assign_user: true,
      })
    ...
    function delete_row() {
      api.delete('/assign/' + delete_row_id.value)
      .then(response=>{
        refresh_grid()
      })
    }
